...
var sendByteMap = [256]byte{ 0x70, 0x2F, 0x40, 0x5F, 0x44, 0x8E,
...
var recvByteMap = [256]byte{ 0x51, 0xA1, 0x9E, 0xB0, 0x1E, 0x83,
...
type MiscCipher struct{
	Key uint
//	RecvByteMap []byte
//	SendByteMap []byte
}

func (this* MiscCipher) DoDecrypt(src []byte) ([]byte,error){
	dest,err := miscDecrypt(src)
	return dest,err
}

func (this* MiscCipher)DoEncrypt(src []byte) ([]byte,error){
	dest,err := miscEncrypt(src)
	return dest,err
}

func mapRecvByte(cbData byte) byte{
	var cbMap byte
	cbMap = recvByteMap[cbData]
	return cbMap
}

// 映射发送数据 与 发送字节映射表 对应有关系
func mapSendByte(cbData byte) byte{
	len := cbData;
	if len >= 255 {
		len = len % 255;
	}
	cbMap := sendByteMap[len]
	return cbMap
}
...
/**
	 ...
	 * @return
*/
func miscDecrypt(src []byte) ([]byte,error){
	for i:=0;i<len(src);i++{
		if src[i] < 0{
			src[i] = mapRecvByte(255 + src[i])
		}else{
			src[i] = mapRecvByte(src[i])
		}
	}
	return src,nil
}
...
/**
	 ...
	 * @return
*/
func miscEncrypt(src []byte) ([]byte,error){
	for i:=0;i<len(src);i++{
		if src[i] < 0{
			src[i] = mapSendByte(255 + src[i])
		}else{
			src[i] = mapSendByte(src[i])
		}
	}
	return src,nil
}
